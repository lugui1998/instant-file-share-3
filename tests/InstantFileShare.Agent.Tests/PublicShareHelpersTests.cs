...
public sealed class PublicShareHelpersTests
{
    ...
    public void BuildFileMetadataPage_AddsCompressionUrlForCompressibleDownloads()
    {
        ...
        Assert.Equal("https://share.example.test/s/file-token/report.csv", page.File.RawDownloadUrl);
        Assert.Equal("https://share.example.test/s/file-token/report.csv?compression=gzip", page.File.CompressedDownloadUrl);
    }
